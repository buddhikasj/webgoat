   ...
   // registry = "gustavoapolinario/docker-test"
    ...
    stages {
        ...
        stage('Scan Aqua'){
            steps{
                script{
                    aqua locationType: 'hosted', registry: 'Docker Hub', hostedImage: "${env.image_name}",  notCompliesCmd: '', onDisallowed: 'fail', hideBase: false, showNegligible: false
        ...
        stage('Deploy') {
            steps {
                echo 'Deploying....'
                sh 'docker run --rm -p 80:8080 --name testcontainer -d buddhikasj88/contrast-httpserver:${env.BUILD_ID}'
            }
        }
